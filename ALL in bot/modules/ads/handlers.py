...
async def send_or_edit_menu(query, context, text, reply_markup, photo_file_id=None):
...
async def ads_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if config and config.get('ad_message'):
        ad_msg_preview = config['ad_message'][:20] + "..." if len(config['ad_message']) > 20 else config['ad_message']
    ...
    dashboard_msg = (
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🤖 **@{bot_username} Control Panel**\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"👥 **Accounts & Content**\n"
        f"├ Hosted Accounts: `{accounts_count}`\n"
        f"└ Ad Message: `{ad_msg_preview}`\n\n"
        f"⚙️ **Engine Configuration**\n"
        f"├ Target Type: `{target.upper()}`\n"
        f"├ Interval: `{interval} mins`\n"
        f"├ Cycles: `{cycles_text}`\n"
        f"└ Auto Reply: `{auto_reply_mode.capitalize()}`\n\n"
        f"📊 **Status:** `{status}`\n"
    )
    
    if target == 'dms':
        dashboard_msg += "\n⚠️ **Warning:** Ads will be sent privately (DM) to the user who sent the latest message in your active groups.\n"
        
    dashboard_msg += "\n╰┈➤ **Select an option below to manage:**"
    
    await send_or_edit_menu(query, context, dashboard_msg, reply_markup, photo_file_id)
...
async def ads_premium_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_or_edit_menu(query, context, msg, InlineKeyboardMarkup(keyboard), None)
...
def setup_ads_handlers(application: Application):
    ...
    userbot_conv_handler = ConversationHandler(
        ...
        fallbacks=[CommandHandler('cancel', ads_dashboard)],
        per_message=False
    )
